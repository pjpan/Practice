...
# load data
df_train = as.data.frame(fread(input="./input/train_users_2.csv"))
df_test = as.data.frame(fread("./input/test_users.csv"))
labels = df_train['country_destination']
df_train = df_train[-grep('country_destination', colnames(df_train))]

sessions = as.data.frame(fread("./input/sessions.csv"))

labels%>%count(country_destination)

View(sessions%>%filter(user_id=='bjjt8pjhuk'))
filter(df_train,id=='d1mm9tcy42')
...
y_train_pred <- predict(xgb, data.matrix(X[,-1]))
train_predictions <- as.data.frame(matrix(y_train_pred, nrow=12))
rownames(train_predictions) <- c('NDF','US','other','FR','CA','GB','ES','IT','PT','NL','DE','AU')
train_predictions_top5 <- as.vector(apply(train_predictions, 2, function(x) names(sort(x)[12:8])))


# save model
xgb.save(xgb,'xgb.model.1')
